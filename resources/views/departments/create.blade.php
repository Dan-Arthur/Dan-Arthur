...
<form method="POST" action="{{ route('departments.store') }}" class="max-w-2xl space-y-6">
@csrf
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="sm:col-span-2">
            ...
            <input type="text" name="name" value="{{ old('name') }}"
                ...
                placeholder="e.g. Science Department" required>
        ...
        <div>
            ...
            <select name="head_id" class="form-select">
                ...
                <option value="{{ $head->id }}" @selected(old('head_id') == $head->id)>{{ $head->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="sm:col-span-2">
            ...
            <textarea name="description" rows="3" class="form-input" placeholder="Optional description…">{{ old('description') }}</textarea>
        </div>
    </div>
</div>
