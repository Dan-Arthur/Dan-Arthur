...
<div class="page-header">
    <div>
        <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-1">
            ...
            <a href="{{ route('assessments.show', $assessment) }}" class="hover:text-blue-600 truncate max-w-xs">
                {{ $assessment->title }}
            </a>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
...
<form method="POST" action="{{ route('assessments.update', $assessment) }}" class="max-w-3xl space-y-6">
@csrf @method('PUT')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <select name="class_id" class="form-select" required>
                ...
                <option value="{{ $class->id }}" @selected(old('class_id', $assessment->class_id) == $class->id)>{{ $class->full_name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="type" class="form-select" required>
                ...
                <option value="{{ $key }}" @selected(old('type', $assessment->type) === $key)>{{ $label }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="number" name="weight" value="{{ old('weight', $assessment->weight) }}"
                step="0.5" min="0" max="100" class="form-input">
        </div>

        <div>
            <label class="form-label">Assessment Date</label>
            <input type="date" name="assessment_date"
                value="{{ old('assessment_date', $assessment->assessment_date?->toDateString()) }}"
                class="form-input">
        </div>
        ...
        <div>
            ...
            <input type="date" name="submission_deadline"
                value="{{ old('submission_deadline', $assessment->submission_deadline?->toDateString()) }}"
                class="form-input">
        </div>
        ...
        <div>
            ...
            <select name="status" class="form-select">
                ...
                <option value="{{ $key }}" @selected(old('status', $assessment->status) === $key)>{{ $info['label'] }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="sm:col-span-2">
            ...
            <textarea name="description" rows="3" class="form-input">{{ old('description', $assessment->description) }}</textarea>
        </div>
    </div>
</div>
...
<div class="flex items-center gap-3">
    ...
    <a href="{{ route('assessments.show', $assessment) }}" class="btn-secondary">Cancel</a>
</div>
</form>
@endsection
